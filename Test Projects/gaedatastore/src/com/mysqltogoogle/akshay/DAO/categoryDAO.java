...
public class categoryDAO { 
	...
	public String[] colNames={"category_id","name","last_update"};
	 ...
	 *
	 */
	public categoryDTO[] select(String str){
		...
		categoryDTO[] categoryValues = new categoryDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			categoryValues[counter]= new categoryDTO();
			String category_id = u1.getProperty("category_id").toString();
			categoryValues[counter].setcategory_id(Integer.parseInt(performOper(category_id)));
			String name = u1.getProperty("name").toString();
			categoryValues[counter].setname((performOper(name)));
			String last_update = u1.getProperty("last_update").toString();
			categoryValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity category = new Entity(tableName,t);
			category.setProperty("category_id",u1.getProperty("category_id"));
			category.setProperty("name",u1.getProperty("name"));
			category.setProperty("last_update",u1.getProperty("last_update"));
			category = UpdateSetQuery(str,category);
			ds.put(category);
		}
	}
	...
	Entity category = new Entity("category",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				category.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				category.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				category.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				category.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				category.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				category.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				category.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity category){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				category.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				category.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				category.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				category.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				category.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				category.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				category.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				category.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
