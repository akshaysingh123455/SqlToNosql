...
public class film_actorDAO { 
	...
	public   String tableName="film_actor";
	public String[] JavaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"actor_id","film_id","last_update"};
	 ...
	 *
	 */
	public film_actorDTO[] select(String str){
		...
		film_actorDTO[] film_actorValues = new film_actorDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			film_actorValues[counter]= new film_actorDTO();
			String actor_id = u1.getProperty("actor_id").toString();
			film_actorValues[counter].setactor_id(Integer.parseInt(performOper(actor_id)));
			String film_id = u1.getProperty("film_id").toString();
			film_actorValues[counter].setfilm_id(Integer.parseInt(performOper(film_id)));
			String last_update = u1.getProperty("last_update").toString();
			film_actorValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return film_actorValues;
	}
	/*
	 * <p>
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity film_actor = new Entity(tableName,t);
			film_actor.setProperty("actor_id",u1.getProperty("actor_id"));
			film_actor.setProperty("film_id",u1.getProperty("film_id"));
			film_actor.setProperty("last_update",u1.getProperty("last_update"));
			film_actor = UpdateSetQuery(str,film_actor);
			ds.put(film_actor);
		}
	}
	...
	Entity film_actor = new Entity("film_actor",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				film_actor.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				film_actor.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				film_actor.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				film_actor.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				film_actor.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				film_actor.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				film_actor.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity film_actor){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				film_actor.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				film_actor.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				film_actor.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				film_actor.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				film_actor.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				film_actor.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				film_actor.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				film_actor.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
