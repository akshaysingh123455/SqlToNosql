...
public class rentalDAO { 
	...
	public   String tableName="rental";
	public String[] JavaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"rental_id","rental_date","inventory_id","customer_id","return_date","staff_id","last_update"};
	...
	public String[] colTypes= {"int","Timestamp","long","int","Timestamp","int","Timestamp"};
	 ...
	 *
	 */
	public rentalDTO[] select(String str){
		...
		rentalDTO[] rentalValues = new rentalDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			rentalValues[counter]= new rentalDTO();
			String rental_id = u1.getProperty("rental_id").toString();
			rentalValues[counter].setrental_id(Integer.parseInt(performOper(rental_id)));
			String rental_date = u1.getProperty("rental_date").toString();
			rentalValues[counter].setrental_date(StringToTimestamp(performOper(rental_date)));
			String inventory_id = u1.getProperty("inventory_id").toString();
			rentalValues[counter].setinventory_id(Long.parseLong(performOper(inventory_id)));
			String customer_id = u1.getProperty("customer_id").toString();
			rentalValues[counter].setcustomer_id(Integer.parseInt(performOper(customer_id)));
			String return_date = u1.getProperty("return_date").toString();
			rentalValues[counter].setreturn_date(StringToTimestamp(performOper(return_date)));
			String staff_id = u1.getProperty("staff_id").toString();
			rentalValues[counter].setstaff_id(Integer.parseInt(performOper(staff_id)));
			String last_update = u1.getProperty("last_update").toString();
			rentalValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return rentalValues;
	}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity rental = new Entity(tableName,t);
			rental.setProperty("rental_id",u1.getProperty("rental_id"));
			rental.setProperty("rental_date",u1.getProperty("rental_date"));
			rental.setProperty("inventory_id",u1.getProperty("inventory_id"));
			rental.setProperty("customer_id",u1.getProperty("customer_id"));
			rental.setProperty("return_date",u1.getProperty("return_date"));
			rental.setProperty("staff_id",u1.getProperty("staff_id"));
			rental.setProperty("last_update",u1.getProperty("last_update"));
			rental = UpdateSetQuery(str,rental);
			ds.put(rental);
		}
	}
	...
	Entity rental = new Entity("rental",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				rental.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				rental.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				rental.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				rental.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				rental.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				rental.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				rental.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity rental){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				rental.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				rental.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				rental.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				rental.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				rental.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				rental.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				rental.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				rental.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
