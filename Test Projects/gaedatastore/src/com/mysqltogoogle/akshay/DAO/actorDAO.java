...
public class actorDAO { 
	...
	public String[] colNames={"actor_id","first_name","last_name","last_update"};
	...
	public String[] colTypes= {"int","String","String","Timestamp"};
	 ...
	 *
	 */
	public actorDTO[] select(String str){
		...
		actorDTO[] actorValues = new actorDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			actorValues[counter]= new actorDTO();
			String actor_id = u1.getProperty("actor_id").toString();
			actorValues[counter].setactor_id(Integer.parseInt(performOper(actor_id)));
			String first_name = u1.getProperty("first_name").toString();
			actorValues[counter].setfirst_name((performOper(first_name)));
			String last_name = u1.getProperty("last_name").toString();
			actorValues[counter].setlast_name((performOper(last_name)));
			String last_update = u1.getProperty("last_update").toString();
			actorValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return actorValues;
	}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity actor = new Entity(tableName,t);
			actor.setProperty("actor_id",u1.getProperty("actor_id"));
			actor.setProperty("first_name",u1.getProperty("first_name"));
			actor.setProperty("last_name",u1.getProperty("last_name"));
			actor.setProperty("last_update",u1.getProperty("last_update"));
			actor = UpdateSetQuery(str,actor);
			ds.put(actor);
		}
	}
	...
	Entity actor = new Entity("actor",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				actor.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				actor.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				actor.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				actor.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				actor.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				actor.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				actor.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity actor){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				actor.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				actor.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				actor.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				actor.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				actor.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				actor.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				actor.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				actor.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
