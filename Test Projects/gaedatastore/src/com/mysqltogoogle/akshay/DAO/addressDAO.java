...
public class addressDAO { 
	...
	public String[] colNames={"address_id","address","address2","district","city_id","postal_code","phone","last_update"};
	...
	public String[] colTypes= {"int","String","String","String","int","String","String","Timestamp"};
	 ...
	 *
	 */
	public addressDTO[] select(String str){
		...
		addressDTO[] addressValues = new addressDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			addressValues[counter]= new addressDTO();
			String address_id = u1.getProperty("address_id").toString();
			addressValues[counter].setaddress_id(Integer.parseInt(performOper(address_id)));
			String address = u1.getProperty("address").toString();
			addressValues[counter].setaddress((performOper(address)));
			String address2 = u1.getProperty("address2").toString();
			addressValues[counter].setaddress2((performOper(address2)));
			String district = u1.getProperty("district").toString();
			addressValues[counter].setdistrict((performOper(district)));
			String city_id = u1.getProperty("city_id").toString();
			addressValues[counter].setcity_id(Integer.parseInt(performOper(city_id)));
			String postal_code = u1.getProperty("postal_code").toString();
			addressValues[counter].setpostal_code((performOper(postal_code)));
			String phone = u1.getProperty("phone").toString();
			addressValues[counter].setphone((performOper(phone)));
			String last_update = u1.getProperty("last_update").toString();
			addressValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity address = new Entity(tableName,t);
			address.setProperty("address_id",u1.getProperty("address_id"));
			address.setProperty("address",u1.getProperty("address"));
			address.setProperty("address2",u1.getProperty("address2"));
			address.setProperty("district",u1.getProperty("district"));
			address.setProperty("city_id",u1.getProperty("city_id"));
			address.setProperty("postal_code",u1.getProperty("postal_code"));
			address.setProperty("phone",u1.getProperty("phone"));
			address.setProperty("last_update",u1.getProperty("last_update"));
			address = UpdateSetQuery(str,address);
			ds.put(address);
		}
	}
	...
	Entity address = new Entity("address",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				address.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				address.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				address.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				address.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				address.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				address.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				address.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity address){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				address.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				address.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				address.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				address.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				address.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				address.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				address.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				address.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
