...
public class storeDAO { 
	...
	public   String tableName="store";
	public String[] JavaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"store_id","manager_staff_id","address_id","last_update"};
	...
	public String[] colTypes= {"int","int","int","Timestamp"};
	 ...
	 *
	 */
	public storeDTO[] select(String str){
		...
		storeDTO[] storeValues = new storeDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			storeValues[counter]= new storeDTO();
			String store_id = u1.getProperty("store_id").toString();
			storeValues[counter].setstore_id(Integer.parseInt(performOper(store_id)));
			String manager_staff_id = u1.getProperty("manager_staff_id").toString();
			storeValues[counter].setmanager_staff_id(Integer.parseInt(performOper(manager_staff_id)));
			String address_id = u1.getProperty("address_id").toString();
			storeValues[counter].setaddress_id(Integer.parseInt(performOper(address_id)));
			String last_update = u1.getProperty("last_update").toString();
			storeValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return storeValues;
	}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity store = new Entity(tableName,t);
			store.setProperty("store_id",u1.getProperty("store_id"));
			store.setProperty("manager_staff_id",u1.getProperty("manager_staff_id"));
			store.setProperty("address_id",u1.getProperty("address_id"));
			store.setProperty("last_update",u1.getProperty("last_update"));
			store = UpdateSetQuery(str,store);
			ds.put(store);
		}
	}
	...
	Entity store = new Entity("store",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				store.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				store.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				store.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				store.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				store.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				store.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				store.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity store){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				store.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				store.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				store.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				store.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				store.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				store.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				store.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				store.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
