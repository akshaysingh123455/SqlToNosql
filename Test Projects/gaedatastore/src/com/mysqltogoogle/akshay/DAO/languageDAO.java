...
public class languageDAO { 
	 ...
	 *
	 */
	public languageDTO[] select(String str){
		...
		languageDTO[] languageValues = new languageDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			languageValues[counter]= new languageDTO();
			String language_id = u1.getProperty("language_id").toString();
			languageValues[counter].setlanguage_id(Integer.parseInt(performOper(language_id)));
			String name = u1.getProperty("name").toString();
			languageValues[counter].setname((performOper(name)));
			String last_update = u1.getProperty("last_update").toString();
			languageValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity language = new Entity(tableName,t);
			language.setProperty("language_id",u1.getProperty("language_id"));
			language.setProperty("name",u1.getProperty("name"));
			language.setProperty("last_update",u1.getProperty("last_update"));
			language = UpdateSetQuery(str,language);
			ds.put(language);
		}
	}
	...
	Entity language = new Entity("language",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				language.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				language.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				language.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				language.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				language.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				language.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				language.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity language){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				language.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				language.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				language.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				language.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				language.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				language.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				language.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				language.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
