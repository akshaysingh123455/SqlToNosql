...
public class film_categoryDAO { 
	...
	public   String tableName="film_category";
	public String[] JavaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"film_id","category_id","last_update"};
	...
	public String[] colTypes= {"int","int","Timestamp"};
	public int primaryKeyId=0;

	public film_categoryDAO() {
	 ...
	 *
	 */
	public film_categoryDTO[] select(String str){
		...
		film_categoryDTO[] film_categoryValues = new film_categoryDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			film_categoryValues[counter]= new film_categoryDTO();
			String film_id = u1.getProperty("film_id").toString();
			film_categoryValues[counter].setfilm_id(Integer.parseInt(performOper(film_id)));
			String category_id = u1.getProperty("category_id").toString();
			film_categoryValues[counter].setcategory_id(Integer.parseInt(performOper(category_id)));
			String last_update = u1.getProperty("last_update").toString();
			film_categoryValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return film_categoryValues;
	}
	/*
	 * <p>
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity film_category = new Entity(tableName,t);
			film_category.setProperty("film_id",u1.getProperty("film_id"));
			film_category.setProperty("category_id",u1.getProperty("category_id"));
			film_category.setProperty("last_update",u1.getProperty("last_update"));
			film_category = UpdateSetQuery(str,film_category);
			ds.put(film_category);
		}
	}
	...
	Entity film_category = new Entity("film_category",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				film_category.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				film_category.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				film_category.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				film_category.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				film_category.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				film_category.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				film_category.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity film_category){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				film_category.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				film_category.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				film_category.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				film_category.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				film_category.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				film_category.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				film_category.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				film_category.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
