...
public class paymentDAO { 
	...
	public String[] colNames={"payment_id","customer_id","staff_id","rental_id","amount","payment_date","last_update"};
	...
	public String[] colTypes= {"int","int","int","int","BigDecimal","Timestamp","Timestamp"};
	 ...
	 *
	 */
	public paymentDTO[] select(String str){
		...
		paymentDTO[] paymentValues = new paymentDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			paymentValues[counter]= new paymentDTO();
			String payment_id = u1.getProperty("payment_id").toString();
			paymentValues[counter].setpayment_id(Integer.parseInt(performOper(payment_id)));
			String customer_id = u1.getProperty("customer_id").toString();
			paymentValues[counter].setcustomer_id(Integer.parseInt(performOper(customer_id)));
			String staff_id = u1.getProperty("staff_id").toString();
			paymentValues[counter].setstaff_id(Integer.parseInt(performOper(staff_id)));
			String rental_id = u1.getProperty("rental_id").toString();
			paymentValues[counter].setrental_id(Integer.parseInt(performOper(rental_id)));
			String amount = u1.getProperty("amount").toString();
			String payment_date = u1.getProperty("payment_date").toString();
			paymentValues[counter].setpayment_date(StringToTimestamp(performOper(payment_date)));
			String last_update = u1.getProperty("last_update").toString();
			paymentValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity payment = new Entity(tableName,t);
			payment.setProperty("payment_id",u1.getProperty("payment_id"));
			payment.setProperty("customer_id",u1.getProperty("customer_id"));
			payment.setProperty("staff_id",u1.getProperty("staff_id"));
			payment.setProperty("rental_id",u1.getProperty("rental_id"));
			payment.setProperty("amount",u1.getProperty("amount"));
			payment.setProperty("payment_date",u1.getProperty("payment_date"));
			payment.setProperty("last_update",u1.getProperty("last_update"));
			payment = UpdateSetQuery(str,payment);
			ds.put(payment);
		}
	}
	...
	Entity payment = new Entity("payment",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				payment.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				payment.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				payment.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				payment.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				payment.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				payment.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				payment.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity payment){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				payment.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				payment.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				payment.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				payment.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				payment.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				payment.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				payment.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				payment.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
