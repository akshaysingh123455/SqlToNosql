...
public class cityDAO { 
	...
	public String[] colNames={"city_id","city","country_id","last_update"};
	...
	public String[] colTypes= {"int","String","int","Timestamp"};
	 ...
	 *
	 */
	public cityDTO[] select(String str){
		...
		cityDTO[] cityValues = new cityDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			cityValues[counter]= new cityDTO();
			String city_id = u1.getProperty("city_id").toString();
			cityValues[counter].setcity_id(Integer.parseInt(performOper(city_id)));
			String city = u1.getProperty("city").toString();
			cityValues[counter].setcity((performOper(city)));
			String country_id = u1.getProperty("country_id").toString();
			cityValues[counter].setcountry_id(Integer.parseInt(performOper(country_id)));
			String last_update = u1.getProperty("last_update").toString();
			cityValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return cityValues;
	}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity city = new Entity(tableName,t);
			city.setProperty("city_id",u1.getProperty("city_id"));
			city.setProperty("city",u1.getProperty("city"));
			city.setProperty("country_id",u1.getProperty("country_id"));
			city.setProperty("last_update",u1.getProperty("last_update"));
			city = UpdateSetQuery(str,city);
			ds.put(city);
		}
	}
	...
	Entity city = new Entity("city",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				city.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				city.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				city.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				city.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				city.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				city.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				city.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity city){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				city.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				city.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				city.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				city.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				city.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				city.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				city.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				city.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
