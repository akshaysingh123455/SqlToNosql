...
public class inventoryDAO { 
	...
	public String[] colNames={"inventory_id","film_id","store_id","last_update"};
	...
	public String[] colTypes= {"long","int","int","Timestamp"};
	 ...
	 *
	 */
	public inventoryDTO[] select(String str){
		...
		inventoryDTO[] inventoryValues = new inventoryDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			inventoryValues[counter]= new inventoryDTO();
			String inventory_id = u1.getProperty("inventory_id").toString();
			inventoryValues[counter].setinventory_id(Long.parseLong(performOper(inventory_id)));
			String film_id = u1.getProperty("film_id").toString();
			inventoryValues[counter].setfilm_id(Integer.parseInt(performOper(film_id)));
			String store_id = u1.getProperty("store_id").toString();
			inventoryValues[counter].setstore_id(Integer.parseInt(performOper(store_id)));
			String last_update = u1.getProperty("last_update").toString();
			inventoryValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity inventory = new Entity(tableName,t);
			inventory.setProperty("inventory_id",u1.getProperty("inventory_id"));
			inventory.setProperty("film_id",u1.getProperty("film_id"));
			inventory.setProperty("store_id",u1.getProperty("store_id"));
			inventory.setProperty("last_update",u1.getProperty("last_update"));
			inventory = UpdateSetQuery(str,inventory);
			ds.put(inventory);
		}
	}
	...
	Entity inventory = new Entity("inventory",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				inventory.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				inventory.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				inventory.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				inventory.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				inventory.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				inventory.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				inventory.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity inventory){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				inventory.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				inventory.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				inventory.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				inventory.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				inventory.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				inventory.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				inventory.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				inventory.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
