...
public class staffDAO { 
	 ...
	 *
	 */
	public staffDTO[] select(String str){
		...
		staffDTO[] staffValues = new staffDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			staffValues[counter]= new staffDTO();
			String staff_id = u1.getProperty("staff_id").toString();
			staffValues[counter].setstaff_id(Integer.parseInt(performOper(staff_id)));
			String first_name = u1.getProperty("first_name").toString();
			staffValues[counter].setfirst_name((performOper(first_name)));
			String last_name = u1.getProperty("last_name").toString();
			staffValues[counter].setlast_name((performOper(last_name)));
			String address_id = u1.getProperty("address_id").toString();
			staffValues[counter].setaddress_id(Integer.parseInt(performOper(address_id)));
			String picture = u1.getProperty("picture").toString();
			staffValues[counter].setpicture((performOper(picture)));
			String email = u1.getProperty("email").toString();
			staffValues[counter].setemail((performOper(email)));
			String store_id = u1.getProperty("store_id").toString();
			staffValues[counter].setstore_id(Integer.parseInt(performOper(store_id)));
			String active = u1.getProperty("active").toString();
			staffValues[counter].setactive(Boolean.parseBoolean(performOper(active)));
			String username = u1.getProperty("username").toString();
			staffValues[counter].setusername((performOper(username)));
			String password = u1.getProperty("password").toString();
			staffValues[counter].setpassword((performOper(password)));
			String last_update = u1.getProperty("last_update").toString();
			staffValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return staffValues;
	}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity staff = new Entity(tableName,t);
			staff.setProperty("staff_id",u1.getProperty("staff_id"));
			staff.setProperty("first_name",u1.getProperty("first_name"));
			staff.setProperty("last_name",u1.getProperty("last_name"));
			staff.setProperty("address_id",u1.getProperty("address_id"));
			staff.setProperty("picture",u1.getProperty("picture"));
			staff.setProperty("email",u1.getProperty("email"));
			staff.setProperty("store_id",u1.getProperty("store_id"));
			staff.setProperty("active",u1.getProperty("active"));
			staff.setProperty("username",u1.getProperty("username"));
			staff.setProperty("password",u1.getProperty("password"));
			staff.setProperty("last_update",u1.getProperty("last_update"));
			staff = UpdateSetQuery(str,staff);
			ds.put(staff);
		}
	}
	...
	Entity staff = new Entity("staff",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				staff.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				staff.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				staff.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				staff.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				staff.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				staff.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				staff.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity staff){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				staff.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				staff.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				staff.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				staff.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				staff.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				staff.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				staff.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				staff.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
