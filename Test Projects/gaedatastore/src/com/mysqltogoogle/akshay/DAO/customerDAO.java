...
public class customerDAO { 
	...
	public String[] colNames={"customer_id","store_id","first_name","last_name","email","address_id","active","create_date","last_update"};
	...
	public String[] colTypes= {"int","int","String","String","String","int","boolean","Timestamp","Timestamp"};
	 ...
	 *
	 */
	public customerDTO[] select(String str){
		...
		customerDTO[] customerValues = new customerDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			customerValues[counter]= new customerDTO();
			String customer_id = u1.getProperty("customer_id").toString();
			customerValues[counter].setcustomer_id(Integer.parseInt(performOper(customer_id)));
			String store_id = u1.getProperty("store_id").toString();
			customerValues[counter].setstore_id(Integer.parseInt(performOper(store_id)));
			String first_name = u1.getProperty("first_name").toString();
			customerValues[counter].setfirst_name((performOper(first_name)));
			String last_name = u1.getProperty("last_name").toString();
			customerValues[counter].setlast_name((performOper(last_name)));
			String email = u1.getProperty("email").toString();
			customerValues[counter].setemail((performOper(email)));
			String address_id = u1.getProperty("address_id").toString();
			customerValues[counter].setaddress_id(Integer.parseInt(performOper(address_id)));
			String active = u1.getProperty("active").toString();
			customerValues[counter].setactive(Boolean.parseBoolean(performOper(active)));
			String create_date = u1.getProperty("create_date").toString();
			customerValues[counter].setcreate_date(StringToTimestamp(performOper(create_date)));
			String last_update = u1.getProperty("last_update").toString();
			customerValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity customer = new Entity(tableName,t);
			customer.setProperty("customer_id",u1.getProperty("customer_id"));
			customer.setProperty("store_id",u1.getProperty("store_id"));
			customer.setProperty("first_name",u1.getProperty("first_name"));
			customer.setProperty("last_name",u1.getProperty("last_name"));
			customer.setProperty("email",u1.getProperty("email"));
			customer.setProperty("address_id",u1.getProperty("address_id"));
			customer.setProperty("active",u1.getProperty("active"));
			customer.setProperty("create_date",u1.getProperty("create_date"));
			customer.setProperty("last_update",u1.getProperty("last_update"));
			customer = UpdateSetQuery(str,customer);
			ds.put(customer);
		}
	}
	...
	Entity customer = new Entity("customer",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				customer.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				customer.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				customer.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				customer.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				customer.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				customer.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				customer.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity customer){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				customer.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				customer.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				customer.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				customer.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				customer.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				customer.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				customer.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				customer.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
