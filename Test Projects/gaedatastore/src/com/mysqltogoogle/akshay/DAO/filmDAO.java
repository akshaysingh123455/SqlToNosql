...
public class filmDAO { 
	...
	public String[] colTypes= {"int","String","String","String","int","int","int","BigDecimal","int","BigDecimal","String","String","Timestamp"};
	 ...
	 *
	 */
	public filmDTO[] select(String str){
		...
		filmDTO[] filmValues = new filmDTO[counter];
		counter=0;
		for(Entity u1:pq.asIterable()){
			filmValues[counter]= new filmDTO();
			String film_id = u1.getProperty("film_id").toString();
			filmValues[counter].setfilm_id(Integer.parseInt(performOper(film_id)));
			String title = u1.getProperty("title").toString();
			filmValues[counter].settitle((performOper(title)));
			String description = u1.getProperty("description").toString();
			filmValues[counter].setdescription((performOper(description)));
			String release_year = u1.getProperty("release_year").toString();
			filmValues[counter].setrelease_year((performOper(release_year)));
			String language_id = u1.getProperty("language_id").toString();
			filmValues[counter].setlanguage_id(Integer.parseInt(performOper(language_id)));
			String original_language_id = u1.getProperty("original_language_id").toString();
			filmValues[counter].setoriginal_language_id(Integer.parseInt(performOper(original_language_id)));
			String rental_duration = u1.getProperty("rental_duration").toString();
			filmValues[counter].setrental_duration(Integer.parseInt(performOper(rental_duration)));
			String rental_rate = u1.getProperty("rental_rate").toString();
			String length = u1.getProperty("length").toString();
			filmValues[counter].setlength(Integer.parseInt(performOper(length)));
			String replacement_cost = u1.getProperty("replacement_cost").toString();
			String rating = u1.getProperty("rating").toString();
			filmValues[counter].setrating((performOper(rating)));
			String special_features = u1.getProperty("special_features").toString();
			filmValues[counter].setspecial_features((performOper(special_features)));
			String last_update = u1.getProperty("last_update").toString();
			filmValues[counter].setlast_update(StringToTimestamp(performOper(last_update)));
			counter++;
		}
		return filmValues;
	}
	...
	public void update(String str){
		...
		for(Entity u1:pq.asIterable()){
			Key k = u1.getKey();
			String t= k.getName();
			Entity film = new Entity(tableName,t);
			film.setProperty("film_id",u1.getProperty("film_id"));
			film.setProperty("title",u1.getProperty("title"));
			film.setProperty("description",u1.getProperty("description"));
			film.setProperty("release_year",u1.getProperty("release_year"));
			film.setProperty("language_id",u1.getProperty("language_id"));
			film.setProperty("original_language_id",u1.getProperty("original_language_id"));
			film.setProperty("rental_duration",u1.getProperty("rental_duration"));
			film.setProperty("rental_rate",u1.getProperty("rental_rate"));
			film.setProperty("length",u1.getProperty("length"));
			film.setProperty("replacement_cost",u1.getProperty("replacement_cost"));
			film.setProperty("rating",u1.getProperty("rating"));
			film.setProperty("special_features",u1.getProperty("special_features"));
			film.setProperty("last_update",u1.getProperty("last_update"));
			film = UpdateSetQuery(str,film);
			ds.put(film);
		}
	}
	...
	Entity film = new Entity("film",performOper(eachValue.get(matchPrimary)));
	...
	break;
			...
			if(colTypes[i] .equals("int")){
				film.setProperty(colNames[i], Integer.parseInt(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("boolean")){
				film.setProperty(colNames[i], Boolean.parseBoolean(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("long")){
				film.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("float")){
				film.setProperty(colNames[i], Float.parseFloat(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("double")){
				film.setProperty(colNames[i], Double.parseDouble(performOper(eachValue.get(i))));
			}
			if(colTypes[i] .equals("String")){
				film.setProperty(colNames[i], performOper(eachValue.get(i)));
			}
			if(colTypes[i] .equals("long")){
				film.setProperty(colNames[i], Long.parseLong(performOper(eachValue.get(i))));
			}
	 ...
	 *
	 */
	public Entity UpdateSetQuery(String str,Entity film){
		...
		for(int i=0;i<eachSet.length;i++){
			...
			if(colTypes[indexFoundColumn] .equals("int")){
				film.setProperty(colNames[indexFoundColumn], Integer.parseInt(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("boolean")){
				film.setProperty(colNames[indexFoundColumn], Boolean.parseBoolean(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("long")){
				film.setProperty(colNames[indexFoundColumn], Long.parseLong(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("float")){
				film.setProperty(colNames[indexFoundColumn], Float.parseFloat(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("double")){
				film.setProperty(colNames[indexFoundColumn], Double.parseDouble(performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("String")){
				film.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Date")){
				film.setProperty(colNames[indexFoundColumn], (performOper(nameValue[1])));
			}
			if(colTypes[indexFoundColumn] .equals("Timestamp")){
				film.setProperty(colNames[indexFoundColumn], StringToTimestamp(performOper(nameValue[1])));
			}
