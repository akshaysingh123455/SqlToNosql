...
public class folder_detailsDAO { 

	public String databaseName="india";
	public String tableName="folder_details";
	public String tempTableName="folder_details";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"folder_id","folder_name","owner_id","parent_folder_id","num_articles","num_followers"};
	public String[] tempColNames={"folder_id","folder_name","owner_id","parent_folder_id","num_articles","num_followers"};
	...
	public String[] colTypes= {"long","String","int","long","int","int"};
	public int primaryKeyId=0;

	public folder_detailsDAO() {
	...
	public ArrayList<folder_detailsDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("folder_id") || ColumnNamesToReturned[i] .equals("folder_name") || ColumnNamesToReturned[i] .equals("owner_id") || ColumnNamesToReturned[i] .equals("parent_folder_id") || ColumnNamesToReturned[i] .equals("num_articles") || ColumnNamesToReturned[i] .equals("num_followers") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						folder_detailsDTO temp = new folder_detailsDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("folder_id")){
								String folder_id = result.get("folder_id").toString();
								temp.setfolder_id(Long.parseLong(performOper(folder_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("folder_name")){
								String folder_name = result.get("folder_name").toString();
								temp.setfolder_name((performOper(folder_name)));
							}
							else if(ColumnNamesToReturned[i] .equals("owner_id")){
								String owner_id = result.get("owner_id").toString();
								temp.setowner_id(Integer.parseInt(performOper(owner_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("parent_folder_id")){
								String parent_folder_id = result.get("parent_folder_id").toString();
								temp.setparent_folder_id(Long.parseLong(performOper(parent_folder_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("num_articles")){
								String num_articles = result.get("num_articles").toString();
								temp.setnum_articles(Integer.parseInt(performOper(num_articles)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("folder_id") || this.performOper(ColumnNamesToReturned[i]) .equals("folder_name") || this.performOper(ColumnNamesToReturned[i]) .equals("owner_id") || this.performOper(ColumnNamesToReturned[i]) .equals("parent_folder_id") || this.performOper(ColumnNamesToReturned[i]) .equals("num_articles") || this.performOper(ColumnNamesToReturned[i]) .equals("num_followers") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("folder_id")){
						arrayValues[counter][i] = cursor.curr().get("folder_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("folder_name")){
						arrayValues[counter][i] = cursor.curr().get("folder_name").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("owner_id")){
						arrayValues[counter][i] = cursor.curr().get("owner_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("parent_folder_id")){
						arrayValues[counter][i] = cursor.curr().get("parent_folder_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_articles")){
						arrayValues[counter][i] = cursor.curr().get("num_articles").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_followers")){
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			folder_detailsDAO test = new folder_detailsDAO();
