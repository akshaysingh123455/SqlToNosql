...
public class article_tagsDAO { 

	public String databaseName="india";
	public String tableName="article_tags";
	public String tempTableName="article_tags";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"tag_id","name","num_followers"};
	public String[] tempColNames={"tag_id","name","num_followers"};
	...
	public String[] colTypes= {"int","String","int"};
	public int primaryKeyId=0;

	public article_tagsDAO() {
	...
	public ArrayList<article_tagsDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("tag_id") || ColumnNamesToReturned[i] .equals("name") || ColumnNamesToReturned[i] .equals("num_followers") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						article_tagsDTO temp = new article_tagsDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("tag_id")){
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("tag_id") || this.performOper(ColumnNamesToReturned[i]) .equals("name") || this.performOper(ColumnNamesToReturned[i]) .equals("num_followers") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			article_tagsDAO test = new article_tagsDAO();
