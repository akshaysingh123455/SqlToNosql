...
public class category_main_indexDAO { 

	public String databaseName="india";
	public String tableName="category_main_index";
	public String tempTableName="category_main_index";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"category_main_index_id","category_index_name"};
	public String[] tempColNames={"category_main_index_id","category_index_name"};
	...
	public String[] colTypes= {"int","String"};
	public int primaryKeyId=0;

	public category_main_indexDAO() {
	...
	public ArrayList<category_main_indexDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("category_main_index_id") || ColumnNamesToReturned[i] .equals("category_index_name") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						category_main_indexDTO temp = new category_main_indexDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("category_main_index_id")){
							...
							else if(ColumnNamesToReturned[i] .equals("category_index_name")){
								String category_index_name = result.get("category_index_name").toString();
								temp.setcategory_index_name((performOper(category_index_name)));
							}
						}
						test.add(temp);
					}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("category_main_index_id") || this.performOper(ColumnNamesToReturned[i]) .equals("category_index_name") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("category_main_index_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("category_index_name").toString();
					}
				else{
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			category_main_indexDAO test = new category_main_indexDAO();
