...
public class social_link_typeDAO { 

	public String databaseName="india";
	public String tableName="social_link_type";
	public String tempTableName="social_link_type";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"social_link_type_id","link_name","link"};
	public String[] tempColNames={"social_link_type_id","link_name","link"};
	...
	public ArrayList<social_link_typeDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("social_link_type_id") || ColumnNamesToReturned[i] .equals("link_name") || ColumnNamesToReturned[i] .equals("link") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						social_link_typeDTO temp = new social_link_typeDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("social_link_type_id")){
							...
							else if(ColumnNamesToReturned[i] .equals("link_name")){
								String link_name = result.get("link_name").toString();
								temp.setlink_name((performOper(link_name)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("social_link_type_id") || this.performOper(ColumnNamesToReturned[i]) .equals("link_name") || this.performOper(ColumnNamesToReturned[i]) .equals("link") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("social_link_type_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("link_name").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("link")){
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			social_link_typeDAO test = new social_link_typeDAO();
