...
public class website_index_followersDAO { 

	public String databaseName="india";
	public String tableName="website_index_followers";
	public String tempTableName="website_index_followers";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"website_index_followers_id","website_id","user_id"};
	public String[] tempColNames={"website_index_followers_id","website_id","user_id"};
	...
	public String[] colTypes= {"long","long","int"};
	public int primaryKeyId=0;

	public website_index_followersDAO() {
	...
	public ArrayList<website_index_followersDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("website_index_followers_id") || ColumnNamesToReturned[i] .equals("website_id") || ColumnNamesToReturned[i] .equals("user_id") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						website_index_followersDTO temp = new website_index_followersDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("website_index_followers_id")){
								String website_index_followers_id = result.get("website_index_followers_id").toString();
								temp.setwebsite_index_followers_id(Long.parseLong(performOper(website_index_followers_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("website_id")){
								String website_id = result.get("website_id").toString();
								temp.setwebsite_id(Long.parseLong(performOper(website_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("user_id")){
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("website_index_followers_id") || this.performOper(ColumnNamesToReturned[i]) .equals("website_id") || this.performOper(ColumnNamesToReturned[i]) .equals("user_id") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("website_index_followers_id")){
						arrayValues[counter][i] = cursor.curr().get("website_index_followers_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("website_id")){
						arrayValues[counter][i] = cursor.curr().get("website_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("user_id")){
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			website_index_followersDAO test = new website_index_followersDAO();
