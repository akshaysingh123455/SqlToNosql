...
public class subcategory_indexDAO { 

	public String databaseName="india";
	public String tableName="subcategory_index";
	public String tempTableName="subcategory_index";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"subcategory_index_id","category_id","subcategory_name"};
	public String[] tempColNames={"subcategory_index_id","category_id","subcategory_name"};
	...
	public String[] colTypes= {"int","int","String"};
	public int primaryKeyId=0;

	public subcategory_indexDAO() {
	...
	public ArrayList<subcategory_indexDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("subcategory_index_id") || ColumnNamesToReturned[i] .equals("category_id") || ColumnNamesToReturned[i] .equals("subcategory_name") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						subcategory_indexDTO temp = new subcategory_indexDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("subcategory_index_id")){
							...
							else if(ColumnNamesToReturned[i] .equals("category_id")){
								String category_id = result.get("category_id").toString();
								temp.setcategory_id(Integer.parseInt(performOper(category_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("subcategory_name")){
								String subcategory_name = result.get("subcategory_name").toString();
								temp.setsubcategory_name((performOper(subcategory_name)));
							}
						}
						test.add(temp);
					}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("subcategory_index_id") || this.performOper(ColumnNamesToReturned[i]) .equals("category_id") || this.performOper(ColumnNamesToReturned[i]) .equals("subcategory_name") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("subcategory_index_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("category_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("subcategory_name")){
						arrayValues[counter][i] = cursor.curr().get("subcategory_name").toString();
					}
				else{
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			subcategory_indexDAO test = new subcategory_indexDAO();
