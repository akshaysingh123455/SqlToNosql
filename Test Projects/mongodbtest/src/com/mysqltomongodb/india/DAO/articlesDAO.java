...
public class articlesDAO { 
	...
	public String tempTableName="articles";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"article_id","name","link","content","thumbnail","big_image","parent_website_id","add_date","num_views","num_ups","num_downs","num_saved","num_comments","one_page_article","primary_link","tag_id"};
	public String[] tempColNames={"article_id","name","link","content","thumbnail","big_image","parent_website_id","add_date","num_views","num_ups","num_downs","num_saved","num_comments","one_page_article","primary_link","tag_id"};
	...
	public String[] colTypes= {"long","String","String","String","String","String","long","Timestamp","int","int","int","int","int","int","long","int"};
	...
	public ArrayList<articlesDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("article_id") || ColumnNamesToReturned[i] .equals("name") || ColumnNamesToReturned[i] .equals("link") || ColumnNamesToReturned[i] .equals("content") || ColumnNamesToReturned[i] .equals("thumbnail") || ColumnNamesToReturned[i] .equals("big_image") || ColumnNamesToReturned[i] .equals("parent_website_id") || ColumnNamesToReturned[i] .equals("add_date") || ColumnNamesToReturned[i] .equals("num_views") || ColumnNamesToReturned[i] .equals("num_ups") || ColumnNamesToReturned[i] .equals("num_downs") || ColumnNamesToReturned[i] .equals("num_saved") || ColumnNamesToReturned[i] .equals("num_comments") || ColumnNamesToReturned[i] .equals("one_page_article") || ColumnNamesToReturned[i] .equals("primary_link") || ColumnNamesToReturned[i] .equals("tag_id") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						articlesDTO temp = new articlesDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("article_id")){
								String article_id = result.get("article_id").toString();
								temp.setarticle_id(Long.parseLong(performOper(article_id)));
							}
							...
							else if(ColumnNamesToReturned[i] .equals("content")){
								String content = result.get("content").toString();
								temp.setcontent((performOper(content)));
							}
							else if(ColumnNamesToReturned[i] .equals("thumbnail")){
								String thumbnail = result.get("thumbnail").toString();
								temp.setthumbnail((performOper(thumbnail)));
							}
							else if(ColumnNamesToReturned[i] .equals("big_image")){
								String big_image = result.get("big_image").toString();
								temp.setbig_image((performOper(big_image)));
							}
							else if(ColumnNamesToReturned[i] .equals("parent_website_id")){
								String parent_website_id = result.get("parent_website_id").toString();
								temp.setparent_website_id(Long.parseLong(performOper(parent_website_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("add_date")){
								String add_date = result.get("add_date").toString();
								temp.setadd_date(StringToTimestamp(performOper(add_date)));
							}
							else if(ColumnNamesToReturned[i] .equals("num_views")){
								String num_views = result.get("num_views").toString();
								temp.setnum_views(Integer.parseInt(performOper(num_views)));
							}
							else if(ColumnNamesToReturned[i] .equals("num_ups")){
								String num_ups = result.get("num_ups").toString();
								temp.setnum_ups(Integer.parseInt(performOper(num_ups)));
							}
							else if(ColumnNamesToReturned[i] .equals("num_downs")){
								String num_downs = result.get("num_downs").toString();
								temp.setnum_downs(Integer.parseInt(performOper(num_downs)));
							}
							else if(ColumnNamesToReturned[i] .equals("num_saved")){
								String num_saved = result.get("num_saved").toString();
								temp.setnum_saved(Integer.parseInt(performOper(num_saved)));
							}
							else if(ColumnNamesToReturned[i] .equals("num_comments")){
								String num_comments = result.get("num_comments").toString();
								temp.setnum_comments(Integer.parseInt(performOper(num_comments)));
							}
							else if(ColumnNamesToReturned[i] .equals("one_page_article")){
								String one_page_article = result.get("one_page_article").toString();
								temp.setone_page_article(Integer.parseInt(performOper(one_page_article)));
							}
							else if(ColumnNamesToReturned[i] .equals("primary_link")){
								String primary_link = result.get("primary_link").toString();
								temp.setprimary_link(Long.parseLong(performOper(primary_link)));
							}
							else if(ColumnNamesToReturned[i] .equals("tag_id")){
								String tag_id = result.get("tag_id").toString();
								temp.settag_id(Integer.parseInt(performOper(tag_id)));
							}
						}
						test.add(temp);
					}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("article_id") || this.performOper(ColumnNamesToReturned[i]) .equals("name") || this.performOper(ColumnNamesToReturned[i]) .equals("link") || this.performOper(ColumnNamesToReturned[i]) .equals("content") || this.performOper(ColumnNamesToReturned[i]) .equals("thumbnail") || this.performOper(ColumnNamesToReturned[i]) .equals("big_image") || this.performOper(ColumnNamesToReturned[i]) .equals("parent_website_id") || this.performOper(ColumnNamesToReturned[i]) .equals("add_date") || this.performOper(ColumnNamesToReturned[i]) .equals("num_views") || this.performOper(ColumnNamesToReturned[i]) .equals("num_ups") || this.performOper(ColumnNamesToReturned[i]) .equals("num_downs") || this.performOper(ColumnNamesToReturned[i]) .equals("num_saved") || this.performOper(ColumnNamesToReturned[i]) .equals("num_comments") || this.performOper(ColumnNamesToReturned[i]) .equals("one_page_article") || this.performOper(ColumnNamesToReturned[i]) .equals("primary_link") || this.performOper(ColumnNamesToReturned[i]) .equals("tag_id") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("article_id")){
						arrayValues[counter][i] = cursor.curr().get("article_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("name")){
						...
						arrayValues[counter][i] = cursor.curr().get("content").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("thumbnail")){
						arrayValues[counter][i] = cursor.curr().get("thumbnail").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("big_image")){
						arrayValues[counter][i] = cursor.curr().get("big_image").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("parent_website_id")){
						arrayValues[counter][i] = cursor.curr().get("parent_website_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("add_date")){
						arrayValues[counter][i] = cursor.curr().get("add_date").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_views")){
						arrayValues[counter][i] = cursor.curr().get("num_views").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_ups")){
						arrayValues[counter][i] = cursor.curr().get("num_ups").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_downs")){
						arrayValues[counter][i] = cursor.curr().get("num_downs").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_saved")){
						arrayValues[counter][i] = cursor.curr().get("num_saved").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_comments")){
						arrayValues[counter][i] = cursor.curr().get("num_comments").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("one_page_article")){
						arrayValues[counter][i] = cursor.curr().get("one_page_article").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("primary_link")){
						arrayValues[counter][i] = cursor.curr().get("primary_link").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("tag_id")){
						arrayValues[counter][i] = cursor.curr().get("tag_id").toString();
					}
				else{
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			articlesDAO test = new articlesDAO();
