...
public class website_indexDAO { 

	public String databaseName="india";
	public String tableName="website_index";
	public String tempTableName="website_index";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"site_id","name","title","link","description","site_logo","category_main_index_id","subcategory_index_id","add_date","num_ups","num_downs","views","no_articles_registered","made_in_india","num_followers"};
	public String[] tempColNames={"site_id","name","title","link","description","site_logo","category_main_index_id","subcategory_index_id","add_date","num_ups","num_downs","views","no_articles_registered","made_in_india","num_followers"};
	...
	public String[] colTypes= {"long","String","String","String","String","String","int","int","Timestamp","int","int","long","int","int","int"};
	public int primaryKeyId=0;

	public website_indexDAO() {
	...
	public ArrayList<website_indexDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("site_id") || ColumnNamesToReturned[i] .equals("name") || ColumnNamesToReturned[i] .equals("title") || ColumnNamesToReturned[i] .equals("link") || ColumnNamesToReturned[i] .equals("description") || ColumnNamesToReturned[i] .equals("site_logo") || ColumnNamesToReturned[i] .equals("category_main_index_id") || ColumnNamesToReturned[i] .equals("subcategory_index_id") || ColumnNamesToReturned[i] .equals("add_date") || ColumnNamesToReturned[i] .equals("num_ups") || ColumnNamesToReturned[i] .equals("num_downs") || ColumnNamesToReturned[i] .equals("views") || ColumnNamesToReturned[i] .equals("no_articles_registered") || ColumnNamesToReturned[i] .equals("made_in_india") || ColumnNamesToReturned[i] .equals("num_followers") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						website_indexDTO temp = new website_indexDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							...
							else if(ColumnNamesToReturned[i] .equals("title")){
								String title = result.get("title").toString();
								temp.settitle((performOper(title)));
							}
							else if(ColumnNamesToReturned[i] .equals("link")){
								String link = result.get("link").toString();
								temp.setlink((performOper(link)));
							}
							else if(ColumnNamesToReturned[i] .equals("description")){
								String description = result.get("description").toString();
								temp.setdescription((performOper(description)));
							}
							else if(ColumnNamesToReturned[i] .equals("site_logo")){
								String site_logo = result.get("site_logo").toString();
								temp.setsite_logo((performOper(site_logo)));
							}
							else if(ColumnNamesToReturned[i] .equals("category_main_index_id")){
								String category_main_index_id = result.get("category_main_index_id").toString();
								temp.setcategory_main_index_id(Integer.parseInt(performOper(category_main_index_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("subcategory_index_id")){
								String subcategory_index_id = result.get("subcategory_index_id").toString();
								temp.setsubcategory_index_id(Integer.parseInt(performOper(subcategory_index_id)));
							}
							...
							else if(ColumnNamesToReturned[i] .equals("views")){
								String views = result.get("views").toString();
								temp.setviews(Long.parseLong(performOper(views)));
							}
							else if(ColumnNamesToReturned[i] .equals("no_articles_registered")){
								String no_articles_registered = result.get("no_articles_registered").toString();
								temp.setno_articles_registered(Integer.parseInt(performOper(no_articles_registered)));
							}
							else if(ColumnNamesToReturned[i] .equals("made_in_india")){
								String made_in_india = result.get("made_in_india").toString();
								temp.setmade_in_india(Integer.parseInt(performOper(made_in_india)));
							}
							else if(ColumnNamesToReturned[i] .equals("num_followers")){
								String num_followers = result.get("num_followers").toString();
								temp.setnum_followers(Integer.parseInt(performOper(num_followers)));
							}
						}
						test.add(temp);
					}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("site_id") || this.performOper(ColumnNamesToReturned[i]) .equals("name") || this.performOper(ColumnNamesToReturned[i]) .equals("title") || this.performOper(ColumnNamesToReturned[i]) .equals("link") || this.performOper(ColumnNamesToReturned[i]) .equals("description") || this.performOper(ColumnNamesToReturned[i]) .equals("site_logo") || this.performOper(ColumnNamesToReturned[i]) .equals("category_main_index_id") || this.performOper(ColumnNamesToReturned[i]) .equals("subcategory_index_id") || this.performOper(ColumnNamesToReturned[i]) .equals("add_date") || this.performOper(ColumnNamesToReturned[i]) .equals("num_ups") || this.performOper(ColumnNamesToReturned[i]) .equals("num_downs") || this.performOper(ColumnNamesToReturned[i]) .equals("views") || this.performOper(ColumnNamesToReturned[i]) .equals("no_articles_registered") || this.performOper(ColumnNamesToReturned[i]) .equals("made_in_india") || this.performOper(ColumnNamesToReturned[i]) .equals("num_followers") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("site_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("title").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("link")){
						arrayValues[counter][i] = cursor.curr().get("link").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("description")){
						arrayValues[counter][i] = cursor.curr().get("description").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("site_logo")){
						arrayValues[counter][i] = cursor.curr().get("site_logo").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("category_main_index_id")){
						arrayValues[counter][i] = cursor.curr().get("category_main_index_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("subcategory_index_id")){
						arrayValues[counter][i] = cursor.curr().get("subcategory_index_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("add_date")){
						...
						arrayValues[counter][i] = cursor.curr().get("views").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("no_articles_registered")){
						arrayValues[counter][i] = cursor.curr().get("no_articles_registered").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("made_in_india")){
						arrayValues[counter][i] = cursor.curr().get("made_in_india").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("num_followers")){
						arrayValues[counter][i] = cursor.curr().get("num_followers").toString();
					}
				else{
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			website_indexDAO test = new website_indexDAO();
