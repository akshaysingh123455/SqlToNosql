...
public class usersDAO { 
	...
	public String tempTableName="users";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"user_id","name","username","email","password","date","ip"};
	public String[] tempColNames={"user_id","name","username","email","password","date","ip"};
	...
	public String[] colTypes= {"int","String","String","String","String","Timestamp","String"};
	...
	public ArrayList<usersDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("user_id") || ColumnNamesToReturned[i] .equals("name") || ColumnNamesToReturned[i] .equals("username") || ColumnNamesToReturned[i] .equals("email") || ColumnNamesToReturned[i] .equals("password") || ColumnNamesToReturned[i] .equals("date") || ColumnNamesToReturned[i] .equals("ip") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						usersDTO temp = new usersDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("user_id")){
								String user_id = result.get("user_id").toString();
								temp.setuser_id(Integer.parseInt(performOper(user_id)));
							}
							...
							else if(ColumnNamesToReturned[i] .equals("date")){
								String date = result.get("date").toString();
								temp.setdate(StringToTimestamp(performOper(date)));
							}
							else if(ColumnNamesToReturned[i] .equals("ip")){
								String ip = result.get("ip").toString();
								temp.setip((performOper(ip)));
							}
						}
						test.add(temp);
					}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("user_id") || this.performOper(ColumnNamesToReturned[i]) .equals("name") || this.performOper(ColumnNamesToReturned[i]) .equals("username") || this.performOper(ColumnNamesToReturned[i]) .equals("email") || this.performOper(ColumnNamesToReturned[i]) .equals("password") || this.performOper(ColumnNamesToReturned[i]) .equals("date") || this.performOper(ColumnNamesToReturned[i]) .equals("ip") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("user_id")){
						arrayValues[counter][i] = cursor.curr().get("user_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("name")){
						...
						arrayValues[counter][i] = cursor.curr().get("date").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("ip")){
						arrayValues[counter][i] = cursor.curr().get("ip").toString();
					}
				else{
