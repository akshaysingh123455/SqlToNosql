...
public class folder_followersDAO { 

	public String databaseName="india";
	public String tableName="folder_followers";
	public String tempTableName="folder_followers";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"folder_id","user_id"};
	public String[] tempColNames={"folder_id","user_id"};
	...
	public String[] colTypes= {"long","int"};
	public int primaryKeyId=0;

	public folder_followersDAO() {
	...
	public ArrayList<folder_followersDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("folder_id") || ColumnNamesToReturned[i] .equals("user_id") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						folder_followersDTO temp = new folder_followersDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("folder_id") || this.performOper(ColumnNamesToReturned[i]) .equals("user_id") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			folder_followersDAO test = new folder_followersDAO();
