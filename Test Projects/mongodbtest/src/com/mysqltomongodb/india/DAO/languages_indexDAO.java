...
public class languages_indexDAO { 

	public String databaseName="india";
	public String tableName="languages_index";
	public String tempTableName="languages_index";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"website_id","language"};
	public String[] tempColNames={"website_id","language"};
	...
	public String[] colTypes= {"long","String"};
	public int primaryKeyId=0;

	public languages_indexDAO() {
	...
	public ArrayList<languages_indexDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("website_id") || ColumnNamesToReturned[i] .equals("language") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						languages_indexDTO temp = new languages_indexDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("website_id")){
								String website_id = result.get("website_id").toString();
								temp.setwebsite_id(Long.parseLong(performOper(website_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("language")){
								String language = result.get("language").toString();
								temp.setlanguage((performOper(language)));
							}
						}
						test.add(temp);
					}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("website_id") || this.performOper(ColumnNamesToReturned[i]) .equals("language") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("website_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("language").toString();
					}
				else{
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			languages_indexDAO test = new languages_indexDAO();
