...
public class article_folder_linkDAO { 

	public String databaseName="india";
	public String tableName="article_folder_link";
	public String tempTableName="article_folder_link";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"article_id","folder_id","description"};
	public String[] tempColNames={"article_id","folder_id","description"};
	...
	public String[] colTypes= {"long","long","String"};
	public int primaryKeyId=0;

	public article_folder_linkDAO() {
	...
	public ArrayList<article_folder_linkDTO> select(String str){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("article_id") || ColumnNamesToReturned[i] .equals("folder_id") || ColumnNamesToReturned[i] .equals("description") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						article_folder_linkDTO temp = new article_folder_linkDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							...
							else if(ColumnNamesToReturned[i] .equals("folder_id")){
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("article_id") || this.performOper(ColumnNamesToReturned[i]) .equals("folder_id") || this.performOper(ColumnNamesToReturned[i]) .equals("description") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
	...
	public BasicDBObject WhereStatement(String str,int[] tagsIndex){
		...
		public ArrayList<String> getValue(String str){
			...
			str = performOper(str);
			article_folder_linkDAO test = new article_folder_linkDAO();
