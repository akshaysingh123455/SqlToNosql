...
public class storeDAO { 
	...
	public String tempTableName="store";
	...
	public String[] tempColNames={"store_id","manager_staff_id","address_id","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<storeDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("store_id") || ColumnNamesToReturned[i] .equals("manager_staff_id") || ColumnNamesToReturned[i] .equals("address_id") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						storeDTO temp = new storeDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("store_id")){
								String store_id = result.get("store_id").toString();
								temp.setstore_id(Integer.parseInt(performOper(store_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("manager_staff_id")){
								String manager_staff_id = result.get("manager_staff_id").toString();
								temp.setmanager_staff_id(Integer.parseInt(performOper(manager_staff_id)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("store_id") || this.performOper(ColumnNamesToReturned[i]) .equals("manager_staff_id") || this.performOper(ColumnNamesToReturned[i]) .equals("address_id") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("store_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("manager_staff_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("address_id")){
