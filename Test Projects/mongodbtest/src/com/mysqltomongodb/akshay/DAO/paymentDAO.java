...
public class paymentDAO { 
	...
	public String tempTableName="payment";
	...
	public String[] tempColNames={"payment_id","customer_id","staff_id","rental_id","amount","payment_date","last_update"};
	...
	public String[] colTypes= {"int","int","int","int","double","Timestamp","Timestamp"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<paymentDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("payment_id") || ColumnNamesToReturned[i] .equals("customer_id") || ColumnNamesToReturned[i] .equals("staff_id") || ColumnNamesToReturned[i] .equals("rental_id") || ColumnNamesToReturned[i] .equals("amount") || ColumnNamesToReturned[i] .equals("payment_date") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						paymentDTO temp = new paymentDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("payment_id")){
								String payment_id = result.get("payment_id").toString();
								temp.setpayment_id(Integer.parseInt(performOper(payment_id)));
							}
							...
							else if(ColumnNamesToReturned[i] .equals("rental_id")){
								String rental_id = result.get("rental_id").toString();
								temp.setrental_id(Integer.parseInt(performOper(rental_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("amount")){
								String amount = result.get("amount").toString();
								temp.setamount(Double.parseDouble(performOper(amount)));
							}
							else if(ColumnNamesToReturned[i] .equals("payment_date")){
								String payment_date = result.get("payment_date").toString();
								temp.setpayment_date(StringToTimestamp(performOper(payment_date)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("payment_id") || this.performOper(ColumnNamesToReturned[i]) .equals("customer_id") || this.performOper(ColumnNamesToReturned[i]) .equals("staff_id") || this.performOper(ColumnNamesToReturned[i]) .equals("rental_id") || this.performOper(ColumnNamesToReturned[i]) .equals("amount") || this.performOper(ColumnNamesToReturned[i]) .equals("payment_date") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("payment_id")){
						arrayValues[counter][i] = cursor.curr().get("payment_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("customer_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("amount").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("payment_date")){
						arrayValues[counter][i] = cursor.curr().get("payment_date").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_update")){
	...
	*
	 ...
	 */
		public ArrayList<String> getValue(String str,String host,int port){
			...
			str = performOper(str);
			paymentDAO test = new paymentDAO();
