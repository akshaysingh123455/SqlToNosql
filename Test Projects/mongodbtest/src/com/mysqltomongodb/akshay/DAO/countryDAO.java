...
public class countryDAO { 
	...
	public String tempTableName="country";
	...
	public String[] tempColNames={"country_id","country","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<countryDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("country_id") || ColumnNamesToReturned[i] .equals("country") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("country_id") || this.performOper(ColumnNamesToReturned[i]) .equals("country") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("country_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("country").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_update")){
	...
	*
	 ...
	 */
		public ArrayList<String> getValue(String str,String host,int port){
			...
			str = performOper(str);
			countryDAO test = new countryDAO();
