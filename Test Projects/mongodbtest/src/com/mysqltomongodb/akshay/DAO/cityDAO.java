...
public class cityDAO { 
	...
	public String tempTableName="city";
	...
	public String[] tempColNames={"city_id","city","country_id","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<cityDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("city_id") || ColumnNamesToReturned[i] .equals("city") || ColumnNamesToReturned[i] .equals("country_id") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						cityDTO temp = new cityDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("city_id")){
								String city_id = result.get("city_id").toString();
								temp.setcity_id(Integer.parseInt(performOper(city_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("city")){
								String city = result.get("city").toString();
								temp.setcity((performOper(city)));
							}
							else if(ColumnNamesToReturned[i] .equals("country_id")){
								String country_id = result.get("country_id").toString();
								temp.setcountry_id(Integer.parseInt(performOper(country_id)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("city_id") || this.performOper(ColumnNamesToReturned[i]) .equals("city") || this.performOper(ColumnNamesToReturned[i]) .equals("country_id") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("city_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("city").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("country_id")){
						arrayValues[counter][i] = cursor.curr().get("country_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_update")){
