...
public class rentalDAO { 
	...
	public String tempTableName="rental";
	...
	public String[] tempColNames={"rental_id","rental_date","inventory_id","customer_id","return_date","staff_id","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<rentalDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("rental_id") || ColumnNamesToReturned[i] .equals("rental_date") || ColumnNamesToReturned[i] .equals("inventory_id") || ColumnNamesToReturned[i] .equals("customer_id") || ColumnNamesToReturned[i] .equals("return_date") || ColumnNamesToReturned[i] .equals("staff_id") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						rentalDTO temp = new rentalDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("rental_id")){
								String rental_id = result.get("rental_id").toString();
								temp.setrental_id(Integer.parseInt(performOper(rental_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("rental_date")){
								String rental_date = result.get("rental_date").toString();
								temp.setrental_date(StringToTimestamp(performOper(rental_date)));
							}
							else if(ColumnNamesToReturned[i] .equals("inventory_id")){
								String inventory_id = result.get("inventory_id").toString();
								temp.setinventory_id(Long.parseLong(performOper(inventory_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("customer_id")){
								String customer_id = result.get("customer_id").toString();
								temp.setcustomer_id(Integer.parseInt(performOper(customer_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("return_date")){
								String return_date = result.get("return_date").toString();
								temp.setreturn_date(StringToTimestamp(performOper(return_date)));
							}
							else if(ColumnNamesToReturned[i] .equals("staff_id")){
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("rental_id") || this.performOper(ColumnNamesToReturned[i]) .equals("rental_date") || this.performOper(ColumnNamesToReturned[i]) .equals("inventory_id") || this.performOper(ColumnNamesToReturned[i]) .equals("customer_id") || this.performOper(ColumnNamesToReturned[i]) .equals("return_date") || this.performOper(ColumnNamesToReturned[i]) .equals("staff_id") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("rental_id")){
						arrayValues[counter][i] = cursor.curr().get("rental_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("rental_date")){
						arrayValues[counter][i] = cursor.curr().get("rental_date").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("inventory_id")){
						arrayValues[counter][i] = cursor.curr().get("inventory_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("customer_id")){
						arrayValues[counter][i] = cursor.curr().get("customer_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("return_date")){
						arrayValues[counter][i] = cursor.curr().get("return_date").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("staff_id")){
