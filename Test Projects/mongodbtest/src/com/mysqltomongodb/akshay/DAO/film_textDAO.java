...
public class film_textDAO { 
	...
	public String tempTableName="film_text";
	...
	public String[] tempColNames={"film_id","title","description"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<film_textDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("film_id") || ColumnNamesToReturned[i] .equals("title") || ColumnNamesToReturned[i] .equals("description") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						film_textDTO temp = new film_textDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("film_id") || this.performOper(ColumnNamesToReturned[i]) .equals("title") || this.performOper(ColumnNamesToReturned[i]) .equals("description") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
	...
	*
	 ...
	 */
		public ArrayList<String> getValue(String str,String host,int port){
			...
			str = performOper(str);
			film_textDAO test = new film_textDAO();
