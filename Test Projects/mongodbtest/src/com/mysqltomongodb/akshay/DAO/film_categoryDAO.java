...
public class film_categoryDAO { 
	...
	public String tempTableName="film_category";
	...
	public String[] tempColNames={"film_id","category_id","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<film_categoryDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("film_id") || ColumnNamesToReturned[i] .equals("category_id") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						film_categoryDTO temp = new film_categoryDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("film_id") || this.performOper(ColumnNamesToReturned[i]) .equals("category_id") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
	...
	*
	 ...
	 */
		public ArrayList<String> getValue(String str,String host,int port){
			...
			str = performOper(str);
			film_categoryDAO test = new film_categoryDAO();
