...
public class addressDAO { 
	...
	public String tempTableName="address";
	...
	public String[] tempColNames={"address_id","address","address2","district","city_id","postal_code","phone","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<addressDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("address_id") || ColumnNamesToReturned[i] .equals("address") || ColumnNamesToReturned[i] .equals("address2") || ColumnNamesToReturned[i] .equals("district") || ColumnNamesToReturned[i] .equals("city_id") || ColumnNamesToReturned[i] .equals("postal_code") || ColumnNamesToReturned[i] .equals("phone") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						addressDTO temp = new addressDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("address_id")){
								String address_id = result.get("address_id").toString();
								temp.setaddress_id(Integer.parseInt(performOper(address_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("address")){
								String address = result.get("address").toString();
								temp.setaddress((performOper(address)));
							}
							else if(ColumnNamesToReturned[i] .equals("address2")){
								String address2 = result.get("address2").toString();
								temp.setaddress2((performOper(address2)));
							}
							else if(ColumnNamesToReturned[i] .equals("district")){
								String district = result.get("district").toString();
								temp.setdistrict((performOper(district)));
							}
							else if(ColumnNamesToReturned[i] .equals("city_id")){
								String city_id = result.get("city_id").toString();
								temp.setcity_id(Integer.parseInt(performOper(city_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("postal_code")){
								String postal_code = result.get("postal_code").toString();
								temp.setpostal_code((performOper(postal_code)));
							}
							else if(ColumnNamesToReturned[i] .equals("phone")){
								String phone = result.get("phone").toString();
								temp.setphone((performOper(phone)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("address_id") || this.performOper(ColumnNamesToReturned[i]) .equals("address") || this.performOper(ColumnNamesToReturned[i]) .equals("address2") || this.performOper(ColumnNamesToReturned[i]) .equals("district") || this.performOper(ColumnNamesToReturned[i]) .equals("city_id") || this.performOper(ColumnNamesToReturned[i]) .equals("postal_code") || this.performOper(ColumnNamesToReturned[i]) .equals("phone") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
			...
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					...
					if(ColumnNamesToReturned[i] .equals("address_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("address").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("address2")){
						arrayValues[counter][i] = cursor.curr().get("address2").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("district")){
						arrayValues[counter][i] = cursor.curr().get("district").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("city_id")){
						arrayValues[counter][i] = cursor.curr().get("city_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("postal_code")){
						arrayValues[counter][i] = cursor.curr().get("postal_code").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("phone")){
						arrayValues[counter][i] = cursor.curr().get("phone").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_update")){
