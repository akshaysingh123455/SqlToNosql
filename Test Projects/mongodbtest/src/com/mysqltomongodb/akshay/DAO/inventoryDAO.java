...
public class inventoryDAO { 
	...
	public String tempTableName="inventory";
	...
	public String[] tempColNames={"inventory_id","film_id","store_id","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<inventoryDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("inventory_id") || ColumnNamesToReturned[i] .equals("film_id") || ColumnNamesToReturned[i] .equals("store_id") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("inventory_id") || this.performOper(ColumnNamesToReturned[i]) .equals("film_id") || this.performOper(ColumnNamesToReturned[i]) .equals("store_id") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
	...
	*
	 ...
	 */
		public ArrayList<String> getValue(String str,String host,int port){
			...
			str = performOper(str);
			inventoryDAO test = new inventoryDAO();
