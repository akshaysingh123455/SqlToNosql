...
public class customerDAO { 
	...
	public String tempTableName="customer";
	...
	public String[] tempColNames={"customer_id","store_id","first_name","last_name","email","address_id","active","create_date","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<customerDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("customer_id") || ColumnNamesToReturned[i] .equals("store_id") || ColumnNamesToReturned[i] .equals("first_name") || ColumnNamesToReturned[i] .equals("last_name") || ColumnNamesToReturned[i] .equals("email") || ColumnNamesToReturned[i] .equals("address_id") || ColumnNamesToReturned[i] .equals("active") || ColumnNamesToReturned[i] .equals("create_date") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						customerDTO temp = new customerDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("customer_id")){
								String customer_id = result.get("customer_id").toString();
								temp.setcustomer_id(Integer.parseInt(performOper(customer_id)));
							}
							...
							else if(ColumnNamesToReturned[i] .equals("create_date")){
								String create_date = result.get("create_date").toString();
								temp.setcreate_date(StringToTimestamp(performOper(create_date)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("customer_id") || this.performOper(ColumnNamesToReturned[i]) .equals("store_id") || this.performOper(ColumnNamesToReturned[i]) .equals("first_name") || this.performOper(ColumnNamesToReturned[i]) .equals("last_name") || this.performOper(ColumnNamesToReturned[i]) .equals("email") || this.performOper(ColumnNamesToReturned[i]) .equals("address_id") || this.performOper(ColumnNamesToReturned[i]) .equals("active") || this.performOper(ColumnNamesToReturned[i]) .equals("create_date") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("customer_id")){
						arrayValues[counter][i] = cursor.curr().get("customer_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("store_id")){
						...
						arrayValues[counter][i] = cursor.curr().get("create_date").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_update")){
