...
public class staffDAO { 
	...
	public String tempTableName="staff";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"staff_id","first_name","last_name","address_id","picture","email","store_id","active","username","password","last_update"};
	public String[] tempColNames={"staff_id","first_name","last_name","address_id","picture","email","store_id","active","username","password","last_update"};
	...
	public String[] colTypes= {"int","String","String","int","String","String","int","boolean","String","String","Timestamp"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<staffDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("staff_id") || ColumnNamesToReturned[i] .equals("first_name") || ColumnNamesToReturned[i] .equals("last_name") || ColumnNamesToReturned[i] .equals("address_id") || ColumnNamesToReturned[i] .equals("picture") || ColumnNamesToReturned[i] .equals("email") || ColumnNamesToReturned[i] .equals("store_id") || ColumnNamesToReturned[i] .equals("active") || ColumnNamesToReturned[i] .equals("username") || ColumnNamesToReturned[i] .equals("password") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						staffDTO temp = new staffDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("staff_id")){
								String staff_id = result.get("staff_id").toString();
								temp.setstaff_id(Integer.parseInt(performOper(staff_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("first_name")){
								String first_name = result.get("first_name").toString();
								temp.setfirst_name((performOper(first_name)));
							}
							else if(ColumnNamesToReturned[i] .equals("last_name")){
								String last_name = result.get("last_name").toString();
								temp.setlast_name((performOper(last_name)));
							}
							else if(ColumnNamesToReturned[i] .equals("address_id")){
								String address_id = result.get("address_id").toString();
								temp.setaddress_id(Integer.parseInt(performOper(address_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("picture")){
								String picture = result.get("picture").toString();
								temp.setpicture((performOper(picture)));
							}
							else if(ColumnNamesToReturned[i] .equals("email")){
								String email = result.get("email").toString();
								temp.setemail((performOper(email)));
							}
							else if(ColumnNamesToReturned[i] .equals("store_id")){
								String store_id = result.get("store_id").toString();
								temp.setstore_id(Integer.parseInt(performOper(store_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("active")){
								String active = result.get("active").toString();
								temp.setactive(Boolean.parseBoolean(performOper(active)));
							}
							else if(ColumnNamesToReturned[i] .equals("username")){
								String username = result.get("username").toString();
								temp.setusername((performOper(username)));
							}
							else if(ColumnNamesToReturned[i] .equals("password")){
								String password = result.get("password").toString();
								temp.setpassword((performOper(password)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("staff_id") || this.performOper(ColumnNamesToReturned[i]) .equals("first_name") || this.performOper(ColumnNamesToReturned[i]) .equals("last_name") || this.performOper(ColumnNamesToReturned[i]) .equals("address_id") || this.performOper(ColumnNamesToReturned[i]) .equals("picture") || this.performOper(ColumnNamesToReturned[i]) .equals("email") || this.performOper(ColumnNamesToReturned[i]) .equals("store_id") || this.performOper(ColumnNamesToReturned[i]) .equals("active") || this.performOper(ColumnNamesToReturned[i]) .equals("username") || this.performOper(ColumnNamesToReturned[i]) .equals("password") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("staff_id")){
						arrayValues[counter][i] = cursor.curr().get("staff_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("first_name")){
						arrayValues[counter][i] = cursor.curr().get("first_name").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_name")){
						arrayValues[counter][i] = cursor.curr().get("last_name").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("address_id")){
						arrayValues[counter][i] = cursor.curr().get("address_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("picture")){
						arrayValues[counter][i] = cursor.curr().get("picture").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("email")){
						arrayValues[counter][i] = cursor.curr().get("email").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("store_id")){
						arrayValues[counter][i] = cursor.curr().get("store_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("active")){
						arrayValues[counter][i] = cursor.curr().get("active").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("username")){
						arrayValues[counter][i] = cursor.curr().get("username").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("password")){
						arrayValues[counter][i] = cursor.curr().get("password").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_update")){
