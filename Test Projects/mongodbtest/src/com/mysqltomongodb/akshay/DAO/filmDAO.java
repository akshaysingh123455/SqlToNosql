...
public class filmDAO { 
	...
	public String tempTableName="film";
	public String[] javaDataTypes={"int","boolean","long","float","double","String","Date","Timestamp","Time"};
	public String[] colNames={"film_id","title","description","release_year","language_id","original_language_id","rental_duration","rental_rate","length","replacement_cost","rating","special_features","last_update"};
	public String[] tempColNames={"film_id","title","description","release_year","language_id","original_language_id","rental_duration","rental_rate","length","replacement_cost","rating","special_features","last_update"};
	...
	public String[] colTypes= {"int","String","String","String","int","int","int","double","int","double","String","String","Timestamp"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<filmDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("film_id") || ColumnNamesToReturned[i] .equals("title") || ColumnNamesToReturned[i] .equals("description") || ColumnNamesToReturned[i] .equals("release_year") || ColumnNamesToReturned[i] .equals("language_id") || ColumnNamesToReturned[i] .equals("original_language_id") || ColumnNamesToReturned[i] .equals("rental_duration") || ColumnNamesToReturned[i] .equals("rental_rate") || ColumnNamesToReturned[i] .equals("length") || ColumnNamesToReturned[i] .equals("replacement_cost") || ColumnNamesToReturned[i] .equals("rating") || ColumnNamesToReturned[i] .equals("special_features") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						filmDTO temp = new filmDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("film_id")){
								String film_id = result.get("film_id").toString();
								temp.setfilm_id(Integer.parseInt(performOper(film_id)));
							}
							...
							else if(ColumnNamesToReturned[i] .equals("release_year")){
								String release_year = result.get("release_year").toString();
								temp.setrelease_year((performOper(release_year)));
							}
							else if(ColumnNamesToReturned[i] .equals("language_id")){
								String language_id = result.get("language_id").toString();
								temp.setlanguage_id(Integer.parseInt(performOper(language_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("original_language_id")){
								String original_language_id = result.get("original_language_id").toString();
								temp.setoriginal_language_id(Integer.parseInt(performOper(original_language_id)));
							}
							else if(ColumnNamesToReturned[i] .equals("rental_duration")){
								String rental_duration = result.get("rental_duration").toString();
								temp.setrental_duration(Integer.parseInt(performOper(rental_duration)));
							}
							else if(ColumnNamesToReturned[i] .equals("rental_rate")){
								String rental_rate = result.get("rental_rate").toString();
								temp.setrental_rate(Double.parseDouble(performOper(rental_rate)));
							}
							else if(ColumnNamesToReturned[i] .equals("length")){
								String length = result.get("length").toString();
								temp.setlength(Integer.parseInt(performOper(length)));
							}
							else if(ColumnNamesToReturned[i] .equals("replacement_cost")){
								String replacement_cost = result.get("replacement_cost").toString();
								temp.setreplacement_cost(Double.parseDouble(performOper(replacement_cost)));
							}
							else if(ColumnNamesToReturned[i] .equals("rating")){
								String rating = result.get("rating").toString();
								temp.setrating((performOper(rating)));
							}
							else if(ColumnNamesToReturned[i] .equals("special_features")){
								String special_features = result.get("special_features").toString();
								temp.setspecial_features((performOper(special_features)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("film_id") || this.performOper(ColumnNamesToReturned[i]) .equals("title") || this.performOper(ColumnNamesToReturned[i]) .equals("description") || this.performOper(ColumnNamesToReturned[i]) .equals("release_year") || this.performOper(ColumnNamesToReturned[i]) .equals("language_id") || this.performOper(ColumnNamesToReturned[i]) .equals("original_language_id") || this.performOper(ColumnNamesToReturned[i]) .equals("rental_duration") || this.performOper(ColumnNamesToReturned[i]) .equals("rental_rate") || this.performOper(ColumnNamesToReturned[i]) .equals("length") || this.performOper(ColumnNamesToReturned[i]) .equals("replacement_cost") || this.performOper(ColumnNamesToReturned[i]) .equals("rating") || this.performOper(ColumnNamesToReturned[i]) .equals("special_features") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("film_id")){
						arrayValues[counter][i] = cursor.curr().get("film_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("title")){
						...
						arrayValues[counter][i] = cursor.curr().get("release_year").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("language_id")){
						arrayValues[counter][i] = cursor.curr().get("language_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("original_language_id")){
						arrayValues[counter][i] = cursor.curr().get("original_language_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("rental_duration")){
						arrayValues[counter][i] = cursor.curr().get("rental_duration").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("rental_rate")){
						arrayValues[counter][i] = cursor.curr().get("rental_rate").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("length")){
						arrayValues[counter][i] = cursor.curr().get("length").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("replacement_cost")){
						arrayValues[counter][i] = cursor.curr().get("replacement_cost").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("rating")){
						arrayValues[counter][i] = cursor.curr().get("rating").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("special_features")){
						arrayValues[counter][i] = cursor.curr().get("special_features").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("last_update")){
