...
public class actorDAO { 
	...
	public String tempTableName="actor";
	...
	public String[] tempColNames={"actor_id","first_name","last_name","last_update"};
 ...
 * <p>
	 ...
	 *
	 */
	public ArrayList<actorDTO> select(String str,String host,int port){
		try{
		...
		int[] mainTagsIndex = mainClauses(str,count);
			...
			if(groupBy== false){
				...
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(ColumnNamesToReturned[i] .equals("actor_id") || ColumnNamesToReturned[i] .equals("first_name") || ColumnNamesToReturned[i] .equals("last_name") || ColumnNamesToReturned[i] .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
				...
				AggregationOutput output = coll.aggregate(forWhere,groupObject,project,sort,offSet,limit);
					 for (DBObject result : output.results()) {
						actorDTO temp = new actorDTO();
						 for(int i=0;i<ColumnNamesToReturned.length;i++){
							int colCounter=0;
							if(ColumnNamesToReturned[i] .equals("actor_id")){
								String actor_id = result.get("actor_id").toString();
								temp.setactor_id(Integer.parseInt(performOper(actor_id)));
							}
				...
				BasicDBObject objectForProject = new BasicDBObject("_id",0);
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					if(this.performOper(ColumnNamesToReturned[i]) .equals("actor_id") || this.performOper(ColumnNamesToReturned[i]) .equals("first_name") || this.performOper(ColumnNamesToReturned[i]) .equals("last_name") || this.performOper(ColumnNamesToReturned[i]) .equals("last_update") ){
						String tempColumnToGet = "$"+ColumnNamesToReturned[i];
						objectForProject.append(ColumnNamesToReturned[i], tempColumnToGet);
					}else{
							for(int j=0;j<this.colNames.length;j++){
		...
		while (cursor.hasNext()){
			cursor.next();
				for(int i=0;i<ColumnNamesToReturned.length;i++){
					int colCounter=0;
					if(ColumnNamesToReturned[i] .equals("actor_id")){
						arrayValues[counter][i] = cursor.curr().get("actor_id").toString();
					}
					else if(ColumnNamesToReturned[i] .equals("first_name")){
